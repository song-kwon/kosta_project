	...
	<ul class="dropdown-menu">
		...
		<c:choose>
			<c:when test="${sessionScope.login.memberId=='udongca' }">
				<li><a
					href="/udongca_project/oneToOneInquiry/oneToOneInquiryListPaging.udc"
					class="menu">1:1문의관리</a></li>
			</c:when>
			<c:when test="${!empty sessionScope.login.memberId }">
				<li><a
					href="/udongca_project/oneToOneInquiry/registerOneToOneInquiryform.udc?codeType=inquiry_type"
					class="menu">1:1문의하기</a></li>
			</c:when>
			<c:otherwise>
				<li><a href="/udongca_project/loginPage.udc" class="menu">1:1문의</a></li>
			</c:otherwise>
		</c:choose>
	</ul></li>
