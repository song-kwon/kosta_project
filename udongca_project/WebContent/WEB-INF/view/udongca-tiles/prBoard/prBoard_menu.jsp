...
<script type="text/javascript">
					...
					function() {
						$
								.ajax({
									...
									"error" : function(xhr) {
										alert("An error occured while loading cafeMenuList.udc: "
												+ xhr.status
												+ " "
												+ xhr.statusText);
									}
	...
	function mapLocation() {
		...
		geocoder
				.addr2coord(
						'${requestScope.prBoard.cafeAddress}',
	...
	function reviewList(page) {
		...
		$
				.ajax({
					...
					"type" : "POST",
					"data" : "cafeNo=${requestScope.prBoard.cafeNo}&pnum="
							+ page,
					"dataType" : "json",
					"success" : function(json) {
						if (!json.list.length) {
							...
							for (var i = 0; i < json.list.length; i++) {
								html += "<tr>";
								html += "<td>" + json.list[i].reviewNo
										+ "</td>";
								html += "<td><a href='javascript:void(0)' onclick='reviewDetail("
										+ json.list[i].reviewNo
										+ ")'>"
										+ json.list[i].reviewTitle
										+ "</a></td>";
								html += "<td>" + json.list[i].reviewDate
										+ "</td>";
								html += "</tr>";
							}
							html += "</table><br>";
							html += ((!json.pageBean.previousPageGroup) ? "◀"
									: "<a href='javascript:void(0)' onclick='reviewList("
											+ (json.pageBean.beginPage - 1)
											+ ")'>◀</a>");
							...
							html += ((!json.pageBean.nextPageGroup) ? "▶"
									: "<a href='javascript:void(0)' onclick='reviewList("
											+ (json.pageBean.endPage + 1)
											+ ")'>▶</a><br>");
						}
					...
					"error" : function(xhr) {
						alert("An error occured in reviewList(): " + xhr.status
								+ " " + xhr.statusText);
					}
				});
	}
	...
	function reviewDetail(reviewNo) {
		...
		$
				.ajax({
					...
					"type" : "POST",
					"data" : "cafeNo=" + $("#cafeNo").val() + "&reviewNo="
							+ reviewNo,
					"dataType" : "json",
					"success" : function(json) {
						...
						$("#content")
								.append(
										"<table><tr><td id='reviewArea'></td></tr><tr><td id='replyArea'></td></tr></table>");

						var writerId = "'" + json.review.memberId + "'";
						reviewImageArray = json.review.reviewFakeImage
								.split(";");
						...
						for (var i = 0; i < 5; i++) {
							html += "<img src='/udongca_project/udongca-image/star"
									+ ((i < json.review.ratingStars) ? "1"
											: "0")
						...
						html += "</td></tr>";
						html += "</table>";
						html += "<button onclick=reviewModifyForm(" + reviewNo
								+ "," + writerId + ")>수정</button>";
						html += "<button onclick=reviewDelete(" + reviewNo
								+ "," + writerId + ")>삭제</button>";
						...
						for (var i = 0; i < reviewImageArray.length - 1; i++) {
							$("#reviewContent")
									.append(
											"<img src='/udongca_project/images/" + reviewImageArray[i] + "' height='300' width='300'><br>");
						}
						$("#reviewContent").append(
								"<pre id='reviewContentText'></pre>");
						$("#reviewContentText")
								.append(
										document
												.createTextNode(json.review.reviewContent));
						...
						for (var group = 1; group < countGroup; group++) {
							for (var i = 0; i < 2; i++) {
								for (var idx = 0; idx < json.reply.length; idx++) {
									if (json.reply[idx].replyGroup == group
											&& !json.reply[idx].parentReply
											&& !i) {
										html += "<tbody class='reply' id='" + json.reply[idx].replyNo + "'>";
										html += "<tr class='" + group + "'><td id='" + json.reply[idx].replyId + "'>"
												+ json.reply[idx].replyId;
										...
										html += "</td></tr>";
										html += "<tr><td class='replyContent'><textarea style='resize:none;border: thin;background: white;' readonly='readonly'>"
												+ json.reply[idx].replyContent
												+ "</textarea></td></tr></tbody>";
									} else if (json.reply[idx].replyGroup == group
											&& json.reply[idx].parentReply && i) {
										...
										html += "</td></tr>";
										html += "<tr><td class='reReplyContent'><textarea style='resize:none;border: thin;background: white;' readonly='readonly'>["
												+ json.reply[idx].targetName
												+ "]"
												+ json.reply[idx].replyContent
												+ "</textarea></td></tr></tbody>";
									}
					...
					"error" : function(xhr) {
						alert("An error occured in reviewDetail(): "
								+ xhr.status + " " + xhr.statusText);
					}
				});
	}
	function reviewWrite() {
		window.location.href = "/udongca_project/review/reviewWriteForm.udc?cafeNo="
				+ $("#cafeNo").val();
	}

	function reviewModifyForm(reviewNo, writerId) {
		window.location.href = "/udongca_project/review/reviewModifyForm.udc?reviewNo="
				+ reviewNo + "&writerId=" + writerId;
	}

	function reviewDelete(reviewNo, writerId) {
		if (window.confirm("정말 삭제하겠습니까?")) {
			window.location.href = "/udongca_project/review/reviewDelete.udc?reviewNo="
					+ reviewNo
					+ "&writerId="
					+ writerId
					+ "&cafeNo="
